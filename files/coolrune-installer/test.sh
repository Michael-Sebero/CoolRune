...
if [ "$choice" = "5" ]; then
retry_pacman 5 pacman -S --noconfirm --needed --overwrite='*'--ignore=nvidia-390xx-utils,lib32-nvidia-390xx-utils linux-cachyos linux-cachyos-headers linux-cachyos-zfs protonup-git linux-cachyos-nvidia-open nvidia-utils nvidia-utils-s6 lib32-nvidia-utils nvidia-settings
fi

# NVIDIA-PROPRIETARY-DESKTOP CHOICE
if [ "$choice" = "6" ]; then
retry_pacman 5 pacman -S --noconfirm --needed --overwrite='*' --ignore=nvidia-390xx-utils,lib32-nvidia-390xx-utils linux-cachyos linux-cachyos-headers linux-cachyos-zfs protonup-git linux-cachyos-nvidia nvidia-utils nvidia-utils-s6 lib32-nvidia-utils nvidia-settings
fi
