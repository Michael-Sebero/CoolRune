...
groupadd -f gamemode && ACTUAL_USER=$(ls -1 /home | grep -v '^root$' | head -n 1) && [ -n "$ACTUAL_USER" ] && usermod -aG gamemode "$ACTUAL_USER" && echo "Added user $ACTUAL_USER to gamemode group" && groups "$ACTUAL_USER" || { echo "Error: No valid user found in /home"; exit 1; } &&
